"""
SF Occupation preprocessing

Reads SF Occupation data, applies the preprocessing steps below and writes the result to files.
...
Output files: 
- occupation_<YYYY-MM-DD>.csv
- occupation_<YYYY-MM-DD>.feather
"""
...
def merge_data(dfs):
    ...
    - harmonize column names
    - add missing `pamko` to the data update file
...
def drop_empty_rows(df):
    ...
    Args:
        df (dataframe): occupation dataframe

    Returns:
        Occupation dataframe with empty rows dropped
    """
...
def drop_duplicated_rows(df):
    ...
    Returns:
        Occupation dataframe with duplicated rows dropped
    """
...
if __name__ == "__main__":
    # Read data
    dfs = read_data(SF_OCCUPATION_DATA_PATHS)
    ...
    # Write data to file
    write_data(df, SF_SOCIOECONOMIC_OUTPUT_DIR, "occupation", "csv")
    write_data(df, SF_SOCIOECONOMIC_OUTPUT_DIR, "occupation", "feather")
